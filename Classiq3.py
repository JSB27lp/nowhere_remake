...
class Character(pygame.sprite.Sprite,CharacterSound):
    def __init__(self):
        ...
        self.surf = character_standing_sheet_surfBLUE.subsurface((0,0,100,150))
    ...
    def update(self):   
        ...
        if self.rubikCube :
            self.rubikCube.rect.midbottom = self.pos
            self.rubikCube.rect.y -= 150
    ...
    def animate(self):
        if self.vel.x == 0 and self.vel.y == 0:
            if self.last_direction == "walk_EAST_Sheet":
                self.surf = character_standing_sheet_surfBLUE.subsurface((100*7,0,100,150))
            elif self.last_direction == "walk_NORTH_EAST_Sheet":
                self.surf = character_standing_sheet_surfBLUE.subsurface((100*6,0,100,150))
            elif self.last_direction == "walk_NORTH_Sheet":
                self.surf = character_standing_sheet_surfBLUE.subsurface((100*5,0,100,150))
            elif self.last_direction == "walk_NORTH_WEST_Sheet":
                self.surf = character_standing_sheet_surfBLUE.subsurface((100*4,0,100,150))
            elif self.last_direction == "walk_WEST_Sheet":
                self.surf = character_standing_sheet_surfBLUE.subsurface((100*3,0,100,150))
            elif self.last_direction == "walk_SOUTH_WEST_Sheet":
                self.surf = character_standing_sheet_surfBLUE.subsurface((100*2,0,100,150))
            elif self.last_direction == "walk_SOUTH_Sheet":
                self.surf = character_standing_sheet_surfBLUE.subsurface((100*1,0,100,150))
            elif self.last_direction == "walk_SOUTH_EAST_Sheet":
                self.surf = character_standing_sheet_surfBLUE.subsurface((100*0,0,100,150))
        elif self.vel.x > 0 and self.vel.y == 0:
            self.surf = walk_EAST_SheetBLUE.subsurface((100*self.index_frame,0,100,150))
            self.last_direction = "walk_EAST_Sheet"
        elif self.vel.x > 0 and self.vel.y < 0:
            self.surf = walk_NORTH_EAST_SheetBLUE.subsurface((100*self.index_frame,0,100,150))
            self.last_direction = "walk_NORTH_EAST_Sheet"
        elif self.vel.x == 0 and self.vel.y < 0:
            self.surf = walk_NORTH_SheetBLUE.subsurface((100*self.index_frame,0,100,150))
            self.last_direction = "walk_NORTH_Sheet"
        elif self.vel.x < 0 and self.vel.y < 0:
            self.surf = walk_NORTH_WEST_SheetBLUE.subsurface((100*self.index_frame,0,100,150))
            self.last_direction = "walk_NORTH_WEST_Sheet"
        elif self.vel.x < 0 and self.vel.y == 0:
            self.surf = walk_WEST_SheetBLUE.subsurface((100*self.index_frame,0,100,150))
            self.last_direction = "walk_WEST_Sheet"
        elif self.vel.x < 0 and self.vel.y > 0:
            self.surf = walk_SOUTH_WEST_SheetBLUE.subsurface((100*self.index_frame,0,100,150))
            self.last_direction = "walk_SOUTH_WEST_Sheet"
        elif self.vel.x == 0 and self.vel.y > 0:
            self.surf = walk_SOUTH_SheetBLUE.subsurface((100*self.index_frame,0,100,150))
            self.last_direction = "walk_SOUTH_Sheet"
        elif self.vel.x > 0 and self.vel.y > 0:
            self.surf = walk_SOUTH_EAST_SheetBLUE.subsurface((100*self.index_frame,0,100,150))
...
class Bot(Character):
    ...
    def move(self,room):
        ...
        if self.pos.y < room.rect.y+427 :
            self.vel.y = +1
        if self.pos.y > room.rect.y+840 :
            self.vel.y = -1
...
class Room(pygame.sprite.Sprite):
    def __init__(self, room_name_):
        ...
        tmp_room_mask = None
        if room_name_ == "3doors":
            tmp_room_mask = pygame.image.load("assets/rooms/"+room_name_+"_mask.png").convert_alpha()
        else :
            tmp_room_mask = pygame.image.load("assets/rooms/test_room_mask.png").convert_alpha()
        ...
        self.bg_color = (10,14,3)

        self.list_bot = []

        self.rubikCube = None
        chanceNumber = random.randint(1,7)
        if chanceNumber == 7 :
            self.rubikCube = AnimatedObject("rubikSheet_litttle2",27,2,(random.randint(self.rect.x+295,self.rect.x+1183),random.randint(self.rect.y+460,self.rect.y+805)))
    ...
    def addBots(self) :
        for i in range(random.randint(0,3)):
            bot = Bot(self)
            self.list_bot.append(bot)

class Door(pygame.sprite.Sprite):
    def __init__(self, door_name):
        super().__init__()  

        self.surf = pygame.image.load("assets/objects/"+door_name+".png").convert_alpha()
